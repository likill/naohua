  ...
  <div id="appChatPage">
    ...
    <div class="header-bar">
      <div class="header-left">
        <h1 class="app-name">{{ appId === '0' ? '新建应用' : (appInfo?.appName || '网站生成器') }}</h1>
        <a-tag v-if="appInfo?.codeGenType" color="blue" class="code-gen-type-tag">
          {{ formatCodeGenType(appInfo.codeGenType) }}
        </a-tag>
      </div>
      <div class="header-right" v-if="appId !== '0'">
    ...
    <div class="main-content">
      ...
      <div class="sidebar">
        <div class="sidebar-header">
          <h3>我的作品</h3>
          <a-button type="primary" size="small" @click="createNewChat">
            <template #icon>
              <PlusOutlined />
            </template>
            新建对话
        ...
        <div class="app-list">
          <div
            ...
            :key="app.id"
            class="app-item"
            :class="{ active: app.id === Number(route.params.id) }"
      ...
      <div class="chat-section">
        ...
        <div class="input-container">
          <div class="input-wrapper">
            <a-tooltip v-if="!canSendMessage" title="无法在别人的作品下对话哦~" placement="top">
              <a-textarea
      ...
      <div class="preview-section">
        ...
        <div class="preview-content">
          <div v-if="(!previewUrl && !isGenerating) || appId === '0'" class="preview-placeholder">
            <div class="placeholder-icon">🌐</div>
            <p>{{ appId === '0' ? '发送消息后将生成网站' : '网站文件生成完成后将在这里展示' }}</p>
          </div>
...
import { listMyAppVoByPage, addApp } from '@/api/appController'
...
const canSendMessage = computed(() => {
  return appId.value === '0' || isOwner.value
})
...
const switchApp = (appId: number) => {
  if (appId !== Number(route.params.id)) {
...
const sendMessage = async () => {
  ...
  if (appId.value === '0') {
    try {
      ...
      if (res.data.code === 0 && res.data.data) {
        const newAppId = String(res.data.data)
        appId.value = newAppId
        // 更新 URL
        router.replace(`/app/chat/${newAppId}`)
        // 重新加载应用信息
        await fetchAppInfo()
      } else {
        message.error('创建应用失败：' + res.data.message)
        messages.value[aiMessageIndex].content = '创建应用失败，请重试'
...
.app-item.active {
  background: linear-gradient(135deg, rgba(220, 20, 60, 0.15) 0%, rgba(205, 92, 92, 0.1) 100%);
...
.preview-placeholder {
  ...
  height: 100%;
  color: #666;
  background: linear-gradient(135deg, rgba(220, 20, 60, 0.02) 0%, rgba(205, 92, 92, 0.04) 100%);
}
